...
void encryption(char* cipher, char* key, int length);

int main(int argc, char* argv[]) {
  //check arguments from cmd line
  if (argc < 2) {
...
// Always listen for new connections
  while(1) {
    ...
    //spawn child to encrypt message
    pid_t child = fork();
    if(child == -1) {
      perror("Hull Breach!");
      ...
      if (strncmp(buffer, "len", 3) == 0) {
        ...
        encryption(ctemp, ktemp, length-1);

        //send the encrypted message back to the client
        charsRead = send(establishedConnectionFD, ctemp, length, 0); // Send success back
        if (charsRead < 0) error("ERROR writing to socket");
